export default async function handler(req: any, res: any) {
  ...
  if (req.methexport default async function handler(req: any, res: any) {
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Methods", "POST, OPTIONS");
  res.setHeader("Access-Control-Allow-Headers", "Content-Type");
...
IMPORTANT: Return ONLY raw JSON. No markdown, no code fences, no explanation.`;
    ...
    // Current free vision models on OpenRouter (updated March 2025)
    const models = [
...
}od === "OPTIONS") return res.status(200).end();
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed" });
  }
